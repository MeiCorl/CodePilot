# Step 1 Checklist — LLM 接入与 TUI 交互
...
- [x] 配置文件不存在时友好提示
  ...
  - 实际：输出 "配置文件不存在: C:\Users\Administrator\.codepilot\config.json" 及创建提示，退出码 1
  - 结论：通过
...
- [x] ContentBlock 到 Anthropic 消息格式转换正确
...
- [x] ContentBlock 到 OpenAI 消息格式转换正确
...
- [x] 配置缺失时优雅退出
  ...
  - 实际：移除配置文件后运行 codepilot.exe，输出 "错误: 配置文件不存在: C:\Users\Administrator\.codepilot\config.json\n请创建配置文件，可参考项目根目录 config/config.example.json" 后退出
  - 结论：通过
...
- [x] ListSessions 返回按更新时间降序排列的会话摘要
