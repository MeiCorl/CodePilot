...
12. **工具执行超时**：所有工具执行受 `context.WithTimeout` 控制，默认 30 秒，可在 `config.json` 的 `tool_execution_timeout_seconds` 字段覆盖；超时后工具返回明确的超时错误给 LLM
...
├── config/
│   └── config.example.json              # 新增 tools 段示例
└── docs/
    └── step2-工具系统集成/
        ├── spec.md
        ├── tasks.md
