...
- **连接池** — `Pool`(src/internal/mcp/session/pool.go)统一管理多个 MCP server session
- **适配器自动注册** — MCP `tools/list` 返回的工具自动注册到 `tool.Registry`(代码在 `src/internal/mcp/adapter/`)
- **指数退避重连** — `reconnect.Backoff` 默认 1s/3s/9s 三次重试(`src/internal/mcp/reconnect/backoff.go`)
- **WebUI MCP 徽标** — 三元状态:`mcpPool==nil` 未启用 / `Initializing()==true` 连接中 / 否则已就绪或已失败

## §2 核心数据结构

- `Transport`(`src/internal/mcp/transport/transport.go`)— 抽象接口,`Connect / Send / Recv / Close / IsAlive`
- `Message = []byte`(transport.go)— 单条 JSON-RPC 消息字节序列
- `ErrClosed / ErrNotConnected`(transport.go)— Transport 已关闭 / 未连接错误
- `StdioConfig`(`src/internal/mcp/transport/stdio.go`)— stdio 传输配置,字段 `Command / Args / Env / Workdir / Stderr / CloseTimeout / MaxLineBytes`
- `stdioTransport`(stdio.go)— stdio 实现,内部 `cmd / stdin / scanner / done / alive / closed`
- `httpTransport`(`src/internal/mcp/transport/http.go`)— HTTP 实现,内部 `url / client / bearerToken / basicAuth / extraHeaders / timeout / sessionID / respCh`
...
- `Pool`(`src/internal/mcp/session/pool.go`)— 连接池,字段 `sessions / unhealthy / initializing atomic.Bool / closed atomic.Bool`
- `Session`(session/pool.go)— 单个 MCP server 会话,封装 Transport + JSON-RPC client
- `Backoff`(`src/internal/mcp/reconnect/backoff.go`)— 指数退避器,字段 `intervals []time.Duration`
...
`Pool.RegisterAndStart`(`src/internal/mcp/session/pool.go`)流程:
...
- **上游**(MCP 模块依赖):
  - `internal/tool.Registry`(`src/internal/tool/registry.go`)— MCP 工具注册目标
...
| 路径 | 角色 |
|------|------|
| `src/internal/mcp/transport/transport.go` | `Transport` 传输抽象接口 |
| `src/internal/mcp/transport/stdio.go` | `stdioTransport.Connect` 启动子进程 |
| `src/internal/mcp/transport/stdio.go` | `stdioTransport.Recv` 短命 goroutine 读 scanner |
| `src/internal/mcp/transport/http.go` | `httpTransport` HTTP 实现 |
| `src/internal/mcp/transport/http.go` | `httpTransport.Recv` 跟随 respCh 变化 |
| `src/internal/mcp/session/pool.go` | `Pool` 连接池 |
| `src/internal/mcp/session/pool.go` | `initializing atomic.Bool` 状态标识 |
| `src/internal/mcp/session/pool.go` | `RegisterAndStart` 同步拉起单个 server |
| `src/internal/mcp/reconnect/backoff.go` | `Backoff` 退避器 |
| `src/internal/mcp/reconnect/backoff.go` | `NewDefaultBackoff` 默认 1s/3s/9s |
