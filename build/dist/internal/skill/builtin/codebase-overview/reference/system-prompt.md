...
- **Builder 装配**(`builder.go:28`)— 注册多个 Source,按 Placement 分发到 SystemBlocks 或 LeadUserMessage
- **7 个 Source**(Step 4 + 10.1 + 10.2 累加):Static / Environment / AgentsMD / MemoryIndex / SkillsIndex / ConfigAwareness / CodebaseAwareness
- **AGENTS.md 双层合并**(`agents_md.go:71`)— 全局 `~/.codepilot/AGENTS.md` + 项目 `<cwd>/AGENTS.md`,按 H2 段落合并,项目级同名段覆盖全局
...
- `Source` interface(`src/internal/engine/prompt/sources/source.go:117`)— `Name() string + Assemble(ctx, env) (Section, error)`
- `Placement`(source.go:20)— `PlacementSystem / PlacementUserMessage`,决定 Section 进入 LLM 请求的哪个位置
- `Section`(source.go:32)— Source 产出的「一段」内容,字段 `Name / Content / Placement / Tokens`
- `SystemBlock`(source.go:51)— 进入 LLM 请求 system 字段的内容,字段 `Text / Cacheable`
- `SourceStat`(source.go:61)— 单个 Source 的 token 开销统计,字段 `Name / Tokens`
- `SystemPrompt`(source.go:76)— Builder.Assemble 最终产物,字段 `SystemBlocks / LeadUserMessage / Stats / TotalTokens`
- `Builder`(`builder.go:28`)— 装配器,字段 `sources []Source / enabled bool`,`enabled=false` 时 Assemble 短路返回零值
- `Env`(`template.Env`,source.go:110 别名)— Source 接收的输入环境参数,字段含 `OS / CWD / GitStatus / Date / Version / AgentsMDPath`
- `StaticSource`(`sources/static.go:147`)— 静态规则(CodePilot 自身角色、工具使用、风格规约等)
- `EnvironmentSource`(`sources/environment.go:35`)— 环境上下文(OS / CWD / Git 状态)
- `AgentsMDSource`(`sources/agents_md.go:52`)— AGENTS.md 双层合并
- `MemoryIndexSource`(`sources/memory_index.go:58`)— 记忆索引注入
- `SkillsIndexSource`(`src/internal/skill/sources/skills_index.go:49`)— Skill 列表索引注入
- `ConfigAwarenessSource`(`sources/config_awareness.go:58`)— Step 10.1 配置自感知(静态常量,无 IO)
- `CodebaseAwarenessSource`(`sources/codebase_awareness.go:63`)— Step 10.2 代码自感知(静态常量,无 IO)

## §3 关键流程

### 3.1 Builder 装配链(`NewBuilder`)

`prompt.NewBuilder(srcs ...Source) *Builder`(`builder.go:42`)按注册顺序追加,`main.go` 装配:
...
`Builder.Assemble(ctx, env) (SystemPrompt, error)`(`builder.go:76`)流程:
...
`AgentsMDSource.Assemble`(`agents_md.go:85`)流程:

1. **resolvePaths**(agents_md.go:124):`globalPath = <home>/.codepilot/AGENTS.md`、`projectPath = <cwd>/AGENTS.md`
...
`EnvironmentSource.Assemble`(`environment.go:51`)流程:

1. 优先用 `env.OS`(handler 预采样);空时 `runtime.GOOS` 兜底
2. 优先用 `env.CWD`;空时现场 `collectCWD`(软链解析)
3. Git 状态:优先用 `env.GitStatus`;空时 `collectGitStatus(cwd)`(`environment.go:133`)
...
5. 拼为 XML 风格结构化文本(environment.go:84):`<environment>OS / CWD / Git branch / Git status / Date / CodePilot version</environment>`

[Why] 任何错误降级为可读字符串:**Why** 环境信息缺失不应阻塞会话启动;Git 命令超时 1s 也只 warn 不影响主流程。

### 3.4 Anthropic Prompt Caching 触发

`Builder.Assemble` 中 `Placement=System` 的 Section 都标记 `Cacheable=true`(`builder.go:114`),Provider 在拼 system 字段时:
...
- **下游被依赖**:
  ...
  - `main.go:611`(`prompt.NewBuilder(...)`)— 装配链入口
...
| 路径 | 角色 |
|------|------|
| `src/internal/engine/prompt/builder.go:28` | `Builder` 装配器 |
| `src/internal/engine/prompt/builder.go:42` | `NewBuilder` 构造函数 |
| `src/internal/engine/prompt/builder.go:76` | `Assemble` 顺序调用 Source |
| `src/internal/engine/prompt/builder.go:114` | SystemBlock 标记 Cacheable=true |
| `src/internal/engine/prompt/sources/source.go:20` | `Placement` 枚举 |
| `src/internal/engine/prompt/sources/source.go:32` | `Section` Source 产出 |
| `src/internal/engine/prompt/sources/source.go:76` | `SystemPrompt` 最终产物 |
| `src/internal/engine/prompt/sources/source.go:117` | `Source` 接口定义 |
| `src/internal/engine/prompt/sources/static.go:147` | `StaticSource` 静态规则 |
| `src/internal/engine/prompt/sources/environment.go:35` | `EnvironmentSource` 环境上下文 |
| `src/internal/engine/prompt/sources/environment.go:133` | `collectGitStatus` Git 状态采集 |
| `src/internal/engine/prompt/sources/agents_md.go:52` | `AgentsMDSource` AGENTS.md 双层合并 |
| `src/internal/engine/prompt/sources/agents_md.go:124` | `resolvePaths` 计算两级路径 |
| `src/internal/engine/prompt/sources/memory_index.go:58` | `MemoryIndexSource` 记忆索引注入 |
| `src/internal/engine/prompt/sources/config_awareness.go:58` | `ConfigAwarenessSource` 配置自感知 |
| `src/internal/engine/prompt/sources/codebase_awareness.go:63` | `CodebaseAwarenessSource` 代码自感知 |
| `src/internal/skill/sources/skills_index.go:49` | `SkillsIndexSource` Skill 索引 |
| `src/main.go:611` | `prompt.NewBuilder(...)` 装配链入口 |