...
- **「索引 + 按需子文档」二级加载** — SKILL.md 是「模块目录索引」(只列文件名 + 一句话);`reference/*.md` 是具体实现原理(单文件 ≤ 16KB)
...
- `ConfigAwarenessSource`(`src/internal/engine/prompt/sources/config_awareness.go:58`)— 配置自感知 Source,无状态 struct,固定返回 `configAwarenessContent` 常量
- `configAwarenessContent`(config_awareness.go:42)— 固定字符串:`~/.codepilot/setting.json + <cwd>/.codepilot/setting.json. ReadFile+EditFile/WriteFile; see skill "config-management".`
- `CodebaseAwarenessSource`(`src/internal/engine/prompt/sources/codebase_awareness.go:63`)— 代码自感知 Source,无状态 struct,固定返回 `codebaseAwarenessContent` 常量
- `codebaseAwarenessContent`(codebase_awareness.go:45)— 固定字符串:`<codebase_awareness>架构/实现:skill "codebase-overview";use_skill+ReadFile 子文档</codebase_awareness>`
- `Skill`(`src/internal/skill/skill.go`)— Skill 数据结构(详见 skill-system.md)
- `Frontmatter`(`src/internal/skill/loader/loader.go:27`)— SKILL.md YAML 头
- `codebase-overview/SKILL.md`(`src/internal/skill/builtin/codebase-overview/SKILL.md`)— 总索引文件(目录索引,不含实现细节)
- `codebase-overview/reference/*.md`— 13 篇 module md(11 已实现 + 2 stub)

## §3 关键流程

### 3.1 SP 自感知段注册

`main.go:611` 在 `prompt.NewBuilder(...)` 调用中按顺序追加:
...
`codebase-overview/SKILL.md` 是「总索引 + 按需子文档」二级加载结构:

1. **第一级(SKILL.md)**:目录索引,只列 13 个模块的文件名 + 一句话简介,**不含**任何实现细节(< 6KB)
2. **第二级(reference/*.md)**:具体实现原理,单文件 ≤ 16KB,共 13 篇
...
[Why] 二级加载而非一级加载:**Why** 13 篇 module md 总大小远超 64KB(单 Skill 截断上限);塞进 SKILL.md 会一次撑爆上下文,且无关请求也会加载全部内容。按需 ReadFile 让 LLM 只读相关模块。

### 3.3 沙箱外 Skill 资源目录可触达

`buildSkillReadRoots(skillWorkdir, skillHomeDir, skillExecDir)`(`src/main.go:355+`)计算 Skill 附加只读根:
...
`SandboxMiddleware`(`src/internal/security/sandbox_middleware.go:183`)在 `perm == tool.PermRead && len(readRoots) > 0` 时调 `ResolveInSandboxWithRoots(pathStr, workdir, readRoots)`,把 Skill 根目录下的所有子文件视为合法可读范围。
...
| 路径 | 角色 |
|------|------|
| `src/internal/engine/prompt/sources/config_awareness.go:42` | `configAwarenessContent` 配置自感知常量 |
| `src/internal/engine/prompt/sources/config_awareness.go:58` | `ConfigAwarenessSource` 配置自感知 Source |
| `src/internal/engine/prompt/sources/codebase_awareness.go:45` | `codebaseAwarenessContent` 代码自感知常量 |
| `src/internal/engine/prompt/sources/codebase_awareness.go:63` | `CodebaseAwarenessSource` 代码自感知 Source |
| `src/internal/skill/builtin/codebase-overview/SKILL.md` | 总索引(目录索引,不含实现细节) |
| `src/internal/skill/builtin/codebase-overview/reference/*.md` | 13 篇 module md(11 已实现 + 2 stub) |
| `src/internal/skill/builtin/config-management/SKILL.md` | 配置管理 Skill(Step 10.1) |
| `src/main.go:355` | `buildSkillReadRoots` Skill 附加只读根 |
| `src/main.go:611` | `prompt.NewBuilder` 装配自感知 Source |
| `src/internal/security/sandbox.go:64` | `ResolveInSandboxWithRoots` 附加只读根机制 |
| `src/internal/security/sandbox_middleware.go:183` | `SandboxMiddleware` 沙箱中间件 |
| `src/internal/security/sandbox_middleware.go:223` | PermRead 时启用附加根 |