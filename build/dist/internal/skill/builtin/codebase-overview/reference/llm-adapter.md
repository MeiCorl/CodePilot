...
- **双 Provider 适配**:`AnthropicProvider`(`src/llm/anthropic.go:26`)与 `OpenAIProvider`(`src/llm/openai.go:25`)
...
- `ContentBlock`(`src/llm/types.go:25`)— 消息内容统一接口,方法 `Type() ContentBlockType` + `ToText() string`
- `TextBlock`(`src/llm/types.go:33`)— 文本块,字段 `Text string`
- `ToolUseBlock`(`src/llm/types.go:52`)— LLM 发出的工具调用,字段 `ID / Name / Input(json.RawMessage)`,对应 Anthropic `tool_use` 与 OpenAI `tool_calls.function`
- `ToolResultBlock`(`src/llm/types.go:77`)— 工具执行结果,字段 `ToolUseID / Content / IsError`,对应 Anthropic `tool_result` 与 OpenAI `role=tool`
- `Message{Role, Content []ContentBlock}`(`src/llm/types.go:119`)— 通用对话消息
...
- `SystemPrompt`(`src/internal/engine/prompt/sources/source.go:76`)— 结构化 SP,含 `SystemBlocks[]` + `LeadUserMessage` + `Stats` + `TotalTokens`
- `AnthropicProvider`(`src/llm/anthropic.go:26`)— Anthropic Claude 适配,字段 `client *anthropic.Client / model / maxTokens / timeout / maxRetries`
- `OpenAIProvider`(`src/llm/openai.go:25`)— OpenAI GPT 适配,同构字段
...
1. `cfg.APIKey + cfg.BaseURL`(非空时)+ `cfg.Model / MaxTokens / Timeout / MaxRetries` 构造 `NewAnthropicProvider(cfg)`(`anthropic.go:36`)或 `NewOpenAIProvider(cfg)`(`openai.go:35`)
...
`AnthropicProvider.convertTools(specs []tool.ToolSpec) []anthropic.ToolUnionParam`(`anthropic.go:60`)流程:
...
`OpenAIProvider.convertTools(specs) []openai.ChatCompletionToolParam`(`openai.go:58`)流程:
...
`AnthropicProvider.convertMessages(messages) []anthropic.MessageParam`(`anthropic.go:96`)按 Role(User/Assistant)+ ContentBlock 类型分发:
...
`OpenAIProvider.convertMessages`(`openai.go:80+`)按 OpenAI 协议转:ToolUse → `tool_calls`,ToolResult → `role=tool`。
...
- `CodebaseAwarenessSource` / `ConfigAwarenessSource` / `StaticSource` 等 `Placement=System` 段默认 `Cacheable=true`(由 `Builder.Assemble` 统一设置,见 `src/internal/engine/prompt/builder.go:114`)
...
- **方案**:每个 chunk 都携带 `LLMStopReason`,流结束由 `manager.logLLMStopReason` 记录(`src/internal/engine/conversation/manager.go:722`)
...
| `src/llm/types.go:25` | `ContentBlock` 接口 + 三种实现 |
| `src/llm/types.go:52` | `ToolUseBlock` 工具调用块 |
| `src/llm/types.go:77` | `ToolResultBlock` 工具结果块 |
| `src/llm/types.go:119` | `Message` 通用消息结构 |
| `src/llm/anthropic.go:26` | `AnthropicProvider` Anthropic 适配 |
| `src/llm/anthropic.go:60` | `convertTools` Anthropic 工具 schema 转换 |
| `src/llm/anthropic.go:96` | `convertMessages` Anthropic 消息转换 |
| `src/llm/openai.go:25` | `OpenAIProvider` OpenAI 适配 |
| `src/llm/openai.go:58` | `convertTools` OpenAI 工具 schema 转换 |
| `src/llm/context_error.go` | `IsContextTooLongError` 撞墙判定 |
| `src/internal/engine/prompt/sources/source.go:76` | `SystemPrompt` 结构化 SP |
| `src/internal/engine/prompt/builder.go:114` | `Builder.Assemble` 把 Placement=System 标记 Cacheable=true |
| `src/internal/engine/conversation/manager.go:520` | `runOneLLM` 调用 provider.StreamChat |
| `src/internal/engine/conversation/manager.go:722` | `logLLMStopReason` 停止原因日志 |