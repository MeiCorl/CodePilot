...
| ------ | ---------------------------------------- |
| 计划总步骤数 | 12（含子步骤后实际更多）                            |
| 已完成步骤数 | 4（Step 1 / Step 1.1 / Step 1.2 / Step 2） |
| 当前最新版本 | V1.0.3                                   |
| 下一步骤   | Step 3 — ReAct 与 Agent Loop 实现           |
| 最近更新   | 2026-06-03                               |
...
| --- | --------------------- | ----- | ----- | -------------------------------- |
| 3   | ReAct 与 Agent Loop 实现 | 引擎层   | ⏳ 待开始 | `docs/step3-ReAct与Agent Loop实现/` |
...
| 架构层       | 已落地                                        | 待落地                                         |
| --------- | ------------------------------------------ | ------------------------------------------- |
| 第 1 层：交互层 | WebUI（HTTP + WebSocket + 富文本渲染）            | 完整命令系统（Step 9）、Skill 系统（Step 10）            |
| 第 2 层：引擎层 | 对话管理（含单轮工具闭环）、System Prompt 雏形             | Agent Loop（Step 3）、完整 System Prompt（Step 4） |
| 第 3 层：工具层 | 工具抽象 + Registry + 5 内置工具 + 路径沙箱 + Bash 黑名单 | MCP（Step 6）、Hook（Step 11）、SubAgent（Step 12） |
