...
description: "了解 CodePilot 自身的架构 / 模块设计 / 实现原理 / 关键流程 — 系统整体架构、WebUI 交互、多 LLM 适配、会话管理、工具系统、Skill 系统、MCP 集成、上下文管理、自动学习记忆、System Prompt 设计、自我感知系统(Step 10.1)、权限管理、Hook 系统、SubAgent(stub)等 14 大模块。当用户问「CodePilot 是怎么实现 / 内部原理 / 怎么工作的 / 设计思路 / 架构 / 流程」「X 模块的 Y 怎么做的」「CodePilot 的 X 在哪个文件」「Step N 是怎么实现的」等任何关于 CodePilot 自身的问题时,加载本 Skill 拿到模块索引,再按需 ReadFile 具体子 md。"
---
...
| 10  | System Prompt 设计         | Builder + 8 Source(Static/Environment/AgentsMD/MemoryIndex/SkillsIndex/ConfigAwareness/CodebaseAwareness/HooksAwareness)、AGENTS.md 双层合并、Anthropic Prompt Caching | `reference/system-prompt.md`       |
...
4. 据子 md 内容回答用户
5. Stub 模块(SubAgent)告知用户「CodePilot 规划中,详见 docs/」
