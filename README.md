...
目前 CodePilot 已经打通了从 WebUI 交互、Agent Loop、工具调用、权限控制、MCP 接入、上下文压缩、自动记忆、Slash 命令、Skill 系统到「配置/代码自感知」的完整主链路（最新版本 V1.8.0）。下图展示了当前系统能力与架构落地情况。
...
- **代码自感知（V1.8.0）**：`CodebaseAwarenessSource` 把「CodePilot 自身原理 → 加载 codebase-overview 内置 Skill」写入常驻 SP（≈48 token）；`codebase-overview` 内置 Skill 采用「总索引（< 6KB）+ 按需子文档（< 16KB/篇）」二级加载,覆盖 11 篇已实现模块 md + 2 篇 stub（Hook/SubAgent），LLM 问到「CodePilot 的 X 模块怎么实现」能基于真实代码回答而非通用设计
...
│   │   │   │   │   └── SKILL.md
│   │   │   │   └── codebase-overview/    #     代码自感知 Skill：总索引 + 11 篇模块 md + 2 篇 stub（Hook/SubAgent）
│   │   │   │       ├── SKILL.md
│   │   │   │       └── reference/*.md
...
> 当前最新版本 **V1.8.0** · 最近更新 **2026-06-26** · 进行中 **—**（10/12 主线步骤完成，下一步 Step 11 Hook 系统）
> 详细进度见 [.harness/PROGRESS.md](.harness/PROGRESS.md)

```
[██████████████████████████████████░░░] 10/12 主线步骤完成（Step 1-9 已完成 + Step 9.1 快捷命令后端化 + Step 10 Skill 系统 + Step 10.1 配置自感知 + Step 10.2 代码自感知，Step 11-12 待开始）
