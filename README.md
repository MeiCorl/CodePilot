...
│   Web UI（HTTP + WebSocket）│ 命令系统 │ Skill 技能系统        │
├─────────────────────────────────────────────────────────────┤
│                    第 2 层：引擎层（Engine）                   │
│   对话管理（Conversation）│ Agent Loop（ReAct）│ 提示词（Prompt）│
├─────────────────────────────────────────────────────────────┤
│                    第 3 层：工具层（Tool）                     │
│   内置工具集 │ MCP 协议 │ Hook 钩子 │ SubAgent 子代理        │
├─────────────────────────────────────────────────────────────┤
│                    第 4 层：记忆层（Memory）                   │
│   上下文管理 │ 会话管理 │ 自动记忆                              │
├─────────────────────────────────────────────────────────────┤
...
│   │   ├── anthropic.go                 #   Anthropic（Claude）协议适配
...
│   │   │       ├── handler.go           #   WebSocket 消息处理主入口
│   │   │       ├── websocket.go         #   WebSocket 连接管理
│   │   │       ├── protocol.go          #   消息协议定义（JSON-RPC 风格）
│   │   │       ├── tool_msg.go          #   工具调用相关的消息构造
│   │   │       ├── browser.go           #   跨平台浏览器调起
│   │   │       └── static/              #   Web UI 前端静态资源（Go embed 嵌入）
│   │   │           ├── index.html
│   │   │           ├── app.js
│   │   │           ├── style.css
│   │   │           └── vendor/          #   第三方库（marked、hljs、DOMPurify 等）
│   │   ├── tool/                        # 工具层（第 3 层）
│   │   │   ├── tool.go                  #   Tool 接口定义与权限分级
│   │   │   ├── tool_spec.go             #   工具规格定义
...
3. **中断操作**：点击输入框旁的取消按钮可中断当前 Agent Loop
4. **会话管理**：
  - 左侧会话列表可查看历史会话
  - 输入 `/new` 创建新会话
  - 输入 `/sessions` 查看所有会话
